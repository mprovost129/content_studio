...
class RecommendationTuningUpdateView(StaffRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        create_tuning_change_log(
            ...
            experiment_status=form.cleaned_data.get("experiment_status") or RecommendationTuningChangeLog.ExperimentStatus.NOT_EXPERIMENT,
...
class RecommendationTuningSimulationView(StaffRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if form and form.is_bound and form.is_valid():
            ...
            selected_keys = form.cleaned_data.get("preset_keys") or [preset.key for preset in PRESETS]
        ...
        if resource:
            ...
            simulations.append({
                "name": f"Active: {active_tuning.name}",
                "description": active_tuning.notes or "Current saved recommendation tuning profile.",
                "preset_key": "active",
                "is_active": True,
                "recommendations": build_resource_cta_recommendations(resource, limit=limit, tuning=active_tuning),
            })
            for key in selected_keys:
                ...
                simulations.append({
                    ...
                    "preset_key": preset.key,
                    "is_active": False,
                    "recommendations": build_resource_cta_recommendations(resource, limit=limit, tuning=simulated_tuning),
                })
...
def apply_recommendation_tuning_preset(request):
    ...
    apply_preset_to_active_tuning(
        preset,
        changed_by=request.user,
        reason=request.POST.get("change_reason", "") or "Applied recommendation tuning preset.",
        request_path=request.path,
        experiment_label=request.POST.get("experiment_label", ""),
        experiment_status=request.POST.get("experiment_status", "") or RecommendationTuningChangeLog.ExperimentStatus.NOT_EXPERIMENT,
        experiment_notes=request.POST.get("experiment_notes", ""),
    )
    messages.success(request, f"Applied the {preset.name} recommendation tuning preset.")
...
class RecommendationTuningRollbackView(StaffRequiredMixin, DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        label = "before-change" if snapshot == "before" else "after-change"
        messages.success(request, f"Restored the {label} recommendation tuning snapshot and logged the rollback.")
    ...
    def get_context_data(self, **kwargs):
        ...
        context["tracked_fields"] = [
            {"name": field, "active": context["active_snapshot"].get(field), "before": self.object.before.get(field), "after": self.object.after.get(field)}
...
class RecommendationTuningHistoryView(StaffRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = RecommendationTuningChangeLog.objects.select_related("tuning", "changed_by")
        action = self.request.GET.get("action", "")
        status = self.request.GET.get("experiment_status", "")
        outcome = self.request.GET.get("experiment_outcome", "")
        label = self.request.GET.get("experiment_label", "").strip()
    ...
    def get_context_data(self, **kwargs):
        ...
        context["experiment_statuses"] = RecommendationTuningChangeLog.ExperimentStatus.choices
        context["experiment_outcomes"] = RecommendationTuningChangeLog.ExperimentOutcome.choices
        context["selected_action"] = self.request.GET.get("action", "")
        context["selected_experiment_status"] = self.request.GET.get("experiment_status", "")
        context["selected_experiment_outcome"] = self.request.GET.get("experiment_outcome", "")
        context["selected_experiment_label"] = self.request.GET.get("experiment_label", "")
        context["latest_change"] = RecommendationTuningChangeLog.objects.order_by("-created_at", "-pk").first()
        context["active_experiments"] = RecommendationTuningChangeLog.objects.filter(experiment_status__in=[RecommendationTuningChangeLog.ExperimentStatus.PLANNED, RecommendationTuningChangeLog.ExperimentStatus.RUNNING]).count()
        context["completed_experiments"] = RecommendationTuningChangeLog.objects.exclude(experiment_status=RecommendationTuningChangeLog.ExperimentStatus.NOT_EXPERIMENT).exclude(experiment_outcome=RecommendationTuningChangeLog.ExperimentOutcome.NOT_RECORDED).count()
        context["total_changes"] = context.get("paginator").count if context.get("paginator") else len(context.get("change_logs", []))
        return context
...
class RecommendationTuningHistoryExportView(StaffRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        queryset = RecommendationTuningChangeLog.objects.select_related("tuning", "changed_by").order_by("-created_at", "-pk")
        action = request.GET.get("action", "")
        status = request.GET.get("experiment_status", "")
        outcome = request.GET.get("experiment_outcome", "")
        label = request.GET.get("experiment_label", "").strip()
        ...
        if label:
            queryset = queryset.filter(experiment_label__icontains=label)
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = 'attachment; filename="recommendation_tuning_change_history.csv"'
        writer = csv.writer(response)
        writer.writerow([
            "created_at",
            "action",
...
class RecommendationTuningExperimentSnapshotListView(StaffRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["window_choices"] = [7, 14, 30, 60]
        context["snapshot_count"] = context.get("paginator").count if context.get("paginator") else len(context.get("snapshots", []))
        context["latest_snapshot"] = RecommendationTuningExperimentSnapshot.objects.order_by("-generated_at").first()
        return context
...
class RecommendationTuningExperimentSnapshotCreateView(StaffRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["form"] = kwargs.get("form") or RecommendationTuningExperimentSnapshotForm()
        context["existing_snapshots"] = self.object.performance_snapshots.select_related("generated_by")[:10]
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        return redirect("studio:recommendation-tuning-experiment-snapshot-detail", pk=snapshot.pk)
...
class RecommendationTuningExperimentSnapshotDetailView(StaffRequiredMixin, DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action != "apply_decision_recommendation":
            messages.error(request, "Unknown snapshot action.")
            return redirect("studio:recommendation-tuning-experiment-snapshot-detail", pk=self.object.pk)
        apply_decision_to_change_log(
            snapshot=self.object,
            user=request.user if request.user.is_authenticated else None,
            note=request.POST.get("decision_note", "").strip(),
        )
        messages.success(request, "Decision recommendation recorded on the tuning experiment.")
        return redirect("studio:recommendation-tuning-experiment-snapshot-detail", pk=self.object.pk)
...
def _group_snapshot_rows_by_section(rows):
    ...
    by_key = {}
    for row in rows:
        section = by_key.setdefault(row["section_key"], {"key": row["section_key"], "label": row["section_label"], "rows": []})
