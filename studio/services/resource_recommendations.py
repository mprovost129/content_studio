...
class ResourceCTARecommendation:
    ...
    def target_type_label(self) -> str:
        return dict(ResourceCTA.TargetType.choices).get(
    ...
    def can_apply(self) -> bool:
        return (
            not self.already_exists
            and self.feedback_status
            != ResourceCTARecommendationFeedback.Status.DISMISSED
        )

    @property
    def is_dismissed(self) -> bool:
        return (
            self.feedback_status == ResourceCTARecommendationFeedback.Status.DISMISSED
        )
    ...
    def is_ignored(self) -> bool:
        return (
            self.feedback_status == ResourceCTARecommendationFeedback.Status.SHOWN
            and self.times_shown > 1
        )
...
def _tokenize(value: str) -> set[str]:
    ...
    return {
        token for token in cleaned.split() if len(token) > 2 and token not in stop_words
    }
...
def _resource_terms(resource: LearningResource) -> set[str]:
    parts = [
        ...
        resource.summary,
        resource.content,
        resource.beginner_tip,
        resource.category.name if resource.category_id else "",
    ]
...
def _lesson_score(
    ...
    if lesson.pk in related_ids:
        ...
        reasons.append(
            f"already related to this resource (+{tuning.related_lesson_weight})"
        )
    ...
    if overlap:
        overlap_score = min(
            len(overlap) * tuning.topic_overlap_weight, tuning.topic_overlap_cap
        )
        score += overlap_score
    ...
    conversions = ResourceLessonConversionEvent.objects.filter(
        resource=resource, lesson=lesson
    ).count()
    if conversions:
        conversion_score = min(
            conversions * tuning.conversion_weight, tuning.conversion_cap
        )
        score += conversion_score
        reasons.append(
            f"{conversions} prior resource-attributed conversion{'s' if conversions != 1 else ''} (+{conversion_score})"
        )

    cta_clicks = ResourceCTAClickEvent.objects.filter(
        resource=resource, target_lesson=lesson
    ).count()
    if cta_clicks:
        click_score = min(cta_clicks * tuning.cta_click_weight, tuning.cta_click_cap)
        score += click_score
        reasons.append(
            f"{cta_clicks} prior CTA click{'s' if cta_clicks != 1 else ''} (+{click_score})"
        )
...
def _rank_lessons(
    ...
    limit: int = 8,
    tuning: RecommendationTuning | None = None,
) -> list[tuple[Lesson, int, list[str]]]:
    tuning = tuning or RecommendationTuning.get_active()
    ...
    queryset = (
        _public_lesson_queryset()
        .select_related("category")
        .prefetch_related("quiz_questions", "code_challenges")
    )
    ...
    for term in sorted(resource_terms)[:12]:
        filters |= (
            ...
            | Q(learning_objective__icontains=term)
            | Q(beginner_takeaway__icontains=term)
        )
    ...
    for lesson in candidates:
        score, reasons = _lesson_score(
            resource, lesson, related_ids, resource_terms, tuning=tuning
        )
        if score > 0:
...
def _cta_exists(
    resource: LearningResource, target_type: str, target_lesson: Lesson | None = None
) -> bool:
...
def _feedback_adjustment(
    ...
    if exact:
        ...
        if exact.status == ResourceCTARecommendationFeedback.Status.ACCEPTED:
            ...
            notes.append(
                f"+{tuning.exact_accepted_boost} because this exact suggestion was accepted"
            )
        elif exact.status == ResourceCTARecommendationFeedback.Status.DISMISSED:
            adjustment -= tuning.exact_dismissed_penalty
            notes.append(
                f"-{tuning.exact_dismissed_penalty} because this exact suggestion was dismissed"
            )
        elif exact.times_shown > 1:
            penalty = min(
                (exact.times_shown - 1) * tuning.ignored_per_show_penalty,
                tuning.ignored_penalty_cap,
            )
            adjustment -= penalty
            notes.append(
                f"deprioritized because it was shown {exact.times_shown} times without action"
            )
    ...
    if resource.difficulty:
        same_resource_type = same_resource_type.filter(
            resource__difficulty=resource.difficulty
        )
    if resource.category_id:
        same_resource_type = same_resource_type.filter(
            resource__category_id=resource.category_id
        )

    accepted_similar = same_resource_type.filter(
        status=ResourceCTARecommendationFeedback.Status.ACCEPTED
    ).count()
    dismissed_similar = same_resource_type.filter(
        status=ResourceCTARecommendationFeedback.Status.DISMISSED
    ).count()
    ignored_similar = same_resource_type.filter(
        status=ResourceCTARecommendationFeedback.Status.SHOWN, times_shown__gt=1
    ).count()

    if accepted_similar:
        boost = min(
            accepted_similar * tuning.similar_accepted_boost,
            tuning.similar_accepted_cap,
        )
        adjustment += boost
        notes.append(
            f"+{boost} from accepted {recommendation.target_type_label.lower()} CTAs on similar resources"
        )
    if dismissed_similar:
        penalty = min(
            dismissed_similar * tuning.similar_dismissed_penalty,
            tuning.similar_dismissed_cap,
        )
        adjustment -= penalty
        notes.append(
            f"-{penalty} from dismissed {recommendation.target_type_label.lower()} CTAs on similar resources"
        )
    if ignored_similar:
        penalty = min(
            ignored_similar * tuning.similar_ignored_penalty, tuning.similar_ignored_cap
        )
        adjustment -= penalty
        notes.append(
            f"-{penalty} from ignored {recommendation.target_type_label.lower()} CTAs on similar resources"
        )
    ...
    if recommendation.target_lesson:
        ...
        accepted_lesson = same_lesson.filter(
            status=ResourceCTARecommendationFeedback.Status.ACCEPTED
        ).count()
        dismissed_lesson = same_lesson.filter(
            status=ResourceCTARecommendationFeedback.Status.DISMISSED
        ).count()
        if accepted_lesson:
            boost = min(
                accepted_lesson * tuning.same_lesson_accepted_boost,
                tuning.same_lesson_accepted_cap,
            )
            adjustment += boost
            notes.append(
                f"+{boost} because this lesson has been accepted as a CTA elsewhere"
            )
        if dismissed_lesson:
            penalty = min(
                dismissed_lesson * tuning.same_lesson_dismissed_penalty,
                tuning.same_lesson_dismissed_cap,
            )
            adjustment -= penalty
            notes.append(
                f"-{penalty} because this lesson has been dismissed as a CTA elsewhere"
            )

    adjustment = _bounded(
        adjustment, tuning.feedback_adjustment_floor, tuning.feedback_adjustment_ceiling
    )
...
def _apply_feedback_adjustments(
    ...
    for recommendation in recommendations:
        ...
        adjustment, notes, feedback_status, times_shown = _feedback_adjustment(
            resource, recommendation, tuning=tuning
        )
        adjusted.append(
            replace(
                recommendation,
...
def build_resource_cta_recommendations(
    resource: LearningResource,
    limit: int = 8,
    tuning: RecommendationTuning | None = None,
) -> list[ResourceCTARecommendation]:
    ...
    for lesson, score, reasons in ranked_lessons[:5]:
        recommendations.append(
            ResourceCTARecommendation(
                ...
                already_exists=_cta_exists(
                    resource, ResourceCTA.TargetType.LESSON, lesson
                ),
            )
        )

    quiz_candidates = [
        (lesson, score, reasons)
        for lesson, score, reasons in ranked_lessons
        if lesson.quiz_questions.filter(is_active=True).exists()
    ]
    for lesson, score, reasons in quiz_candidates[:2]:
        recommendations.append(
            ResourceCTARecommendation(
                ...
                already_exists=_cta_exists(
                    resource, ResourceCTA.TargetType.QUIZ, lesson
                ),
            )
        )
    ...
    for lesson, score, reasons in challenge_candidates[:2]:
        recommendations.append(
            ResourceCTARecommendation(
                ...
                already_exists=_cta_exists(
                    resource, ResourceCTA.TargetType.CHALLENGE, lesson
                ),
            )
        )
    ...
    if resource.pdf_download_enabled:
        recommendations.append(
            ResourceCTARecommendation(
                ...
                if not resource.pdf_requires_email
                else "Unlock the PDF",
                score=tuning.pdf_lead_magnet_bonus
                if resource.pdf_requires_email
                else tuning.pdf_open_bonus,
                reasons=(
    ...
    recommendations = _apply_feedback_adjustments(
        resource, recommendations, tuning=tuning
    )
    recommendations.sort(
        key=lambda item: (-item.score, item.already_exists, item.title.lower())
    )
    return recommendations[:limit]


def attach_recommendation_feedback(
    resource: LearningResource,
    recommendations: list[ResourceCTARecommendation],
    user=None,
) -> list[ResourceCTARecommendation]:
    ...
    feedback_by_key = {
        ...
        for item in ResourceCTARecommendationFeedback.objects.filter(
            resource=resource,
            recommendation_key__in=[
                recommendation.key for recommendation in recommendations
            ],
        )
    }
    ...
    for recommendation in recommendations:
        ...
        if feedback:
            if feedback.status == ResourceCTARecommendationFeedback.Status.SHOWN:
                ...
                feedback.reasons = list(recommendation.reasons) + list(
                    recommendation.ranking_notes
                )
            ...
            feedback = ResourceCTARecommendationFeedback.objects.create(
                ...
                + list(recommendation.ranking_notes),
                status=ResourceCTARecommendationFeedback.Status.SHOWN,
                first_seen_at=now,
                last_seen_at=now,
                created_by=user_obj,
        ...
        annotated.append(
            ResourceCTARecommendation(
                ...
                or recommendation.score - recommendation.feedback_adjustment,
...
def mark_recommendation_dismissed(
    resource: LearningResource, recommendation_key: str, user=None
) -> ResourceCTARecommendationFeedback:
    recommendations = {
        item.key: item
        for item in build_resource_cta_recommendations(resource, limit=20)
    }
...
def create_cta_from_recommendation(
    resource: LearningResource, recommendation_key: str, user=None
) -> ResourceCTA:
    recommendations = {
        item.key: item
    ...
    current_max = (
        resource.cta_blocks.order_by("-position")
