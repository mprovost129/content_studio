...
from studio.models import GraphicAsset, GraphicTemplate, Lesson, LessonBlock
...
def _first_block(lesson: Lesson, *block_types: str) -> LessonBlock | None:
    return lesson.blocks.filter(block_type__in=block_types).order_by("position", "pk").first()
...
def _short_summary(lesson: Lesson) -> str:
    return lesson.summary or lesson.learning_objective or f"Learn one beginner-friendly Python idea: {lesson.title}."


def _takeaway(lesson: Lesson) -> str:
    return lesson.beginner_takeaway or "Focus on one small idea, run the code, and read the output before moving on."


def _mistake(lesson: Lesson) -> str:
    return lesson.common_mistake or "A common beginner mistake is changing several things at once. Change one line, run the code, and check what changed."
...
SOCIAL_CAROUSEL_TEMPLATES: tuple[SocialCarouselTemplate, ...] = (
    SocialCarouselTemplate(
        ...
        blocks=(
            CarouselBlock(LessonBlock.BlockType.HEADING, "Slide 1 hook", "What does this Python concept actually mean?"),
            CarouselBlock(LessonBlock.BlockType.TEXT, "Plain-English idea", "Use this slide to explain the concept without jargon. Make it feel approachable for someone brand new."),
    ...
    SocialCarouselTemplate(
        ...
        blocks=(
            CarouselBlock(LessonBlock.BlockType.HEADING, "Beginner mistake", "A mistake new Python learners make with this topic"),
            ...
            CarouselBlock(LessonBlock.BlockType.TEXT, "Slow down and inspect", "Read the variable names, data types, punctuation, and indentation before changing the code."),
            CarouselBlock(LessonBlock.BlockType.CALLOUT, "Fixing habit", "Change one thing at a time, run the code, and compare the output."),
        ),
    ),
    SocialCarouselTemplate(
        key="spot_the_bug",
        ...
        blocks=(
            CarouselBlock(LessonBlock.BlockType.HEADING, "Spot the bug", "Can you find the issue before running it?"),
            CarouselBlock(LessonBlock.BlockType.CODE, "Buggy or suspicious code", "{code}"),
            CarouselBlock(LessonBlock.BlockType.CALLOUT, "Clue", "Look closely at names, quotes, parentheses, data types, and indentation."),
            CarouselBlock(LessonBlock.BlockType.OUTPUT, "Expected direction", "{output}"),
            CarouselBlock(LessonBlock.BlockType.CHALLENGE, "Comment your answer", "What would you change first? Explain it in one sentence."),
        ),
    ),
    SocialCarouselTemplate(
        ...
        blocks=(
            CarouselBlock(LessonBlock.BlockType.HEADING, "Python output quiz", "What will this code print?"),
            CarouselBlock(LessonBlock.BlockType.CODE, "Read the code first", "{code}"),
            CarouselBlock(LessonBlock.BlockType.QUIZ, "Make your prediction", "Do not run it yet. Say the output out loud, then check the reveal."),
            CarouselBlock(LessonBlock.BlockType.OUTPUT, "Answer", "{output}"),
            CarouselBlock(LessonBlock.BlockType.CALLOUT, "Why this matters", "{takeaway}"),
        ),
    ),
    SocialCarouselTemplate(
        key="three_things",
        ...
        blocks=(
            CarouselBlock(LessonBlock.BlockType.HEADING, "3 things to remember", "Save these beginner Python reminders"),
            CarouselBlock(LessonBlock.BlockType.LIST, "The reminders", "- Start with one small example.\n- Run the code before changing more.\n- Read the output and error messages carefully."),
            CarouselBlock(LessonBlock.BlockType.CODE, "Example", "{code}"),
            CarouselBlock(LessonBlock.BlockType.OUTPUT, "Output", "{output}"),
            CarouselBlock(LessonBlock.BlockType.CALLOUT, "Next step", "Try changing one value in the code and run it again."),
        ),
    ),
)
...
def get_social_carousel_template_choices() -> list[tuple[str, str]]:
    return [(template.key, f"{template.name} — {template.purpose}") for template in SOCIAL_CAROUSEL_TEMPLATES]


def get_social_carousel_template(key: str) -> SocialCarouselTemplate | None:
    return next((template for template in SOCIAL_CAROUSEL_TEMPLATES if template.key == key), None)
...
def _render_placeholder(content: str, lesson: Lesson) -> str:
    values = {
        ...
        "objective": lesson.learning_objective or f"Understand {lesson.title} well enough to explain it and run a small example.",
...
def ensure_graphic_template(template: SocialCarouselTemplate) -> GraphicTemplate:
    graphic_template, _ = GraphicTemplate.objects.get_or_create(
        ...
        defaults={
            ...
            "configuration": {"source": "social_carousel_template", "template_key": template.key},
            "is_active": True,
        },
    )
    changed = False
    ...
    if not graphic_template.configuration:
        graphic_template.configuration = {"source": "social_carousel_template", "template_key": template.key}
        changed = True
    if changed:
        graphic_template.save(update_fields=("description", "configuration", "updated_at"))
...
def apply_social_carousel_template_to_lesson(
    ...
    *,
    ...
    with transaction.atomic():
        next_position = (lesson.blocks.aggregate(maximum=Max("position"))["maximum"] or 0) + 1
        blocks_created = 0
    ...
    if generate_now and output_formats:
        assets_created = len(generate_graphics(lesson, graphic_template, output_formats))
